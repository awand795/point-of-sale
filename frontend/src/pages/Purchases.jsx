...
const Purchases = () => {
    ...
    return (
        <div className="space-y-5">
            <div><h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">{t('sidebar.purchases')}</h1><p className="text-sm text-slate-500 dark:text-slate-400 font-medium">Manage purchase orders and stock receiving</p></div>
            ...
            <div className="flex items-center gap-4 bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                <div className="flex items-center gap-2"><Filter size={14} className="text-slate-400" /><span className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase">Status</span></div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-xl shadow-slate-200/40 dark:shadow-none border border-slate-100 dark:border-slate-700/50 overflow-hidden">
                <table className="min-w-full">
                    <thead><tr className="border-b border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50">
                        <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Invoice</th>
                        <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Supplier</th>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-700/50">
                        {purchases.length === 0 ? (<tr><td colSpan="5" className="px-5 py-12 text-center"><Package size={32} className="mx-auto text-slate-300 dark:text-slate-600 mb-2" /><p className="text-sm text-slate-400 dark:text-slate-500">No purchase orders found</p></td></tr>) : purchases.map((p) => (
                            <tr key={p.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition">
                                <td className="px-5 py-3.5"><p className="text-sm font-semibold text-slate-800 dark:text-slate-200">{p.invoice_number}</p><p className="text-[10px] text-slate-400 dark:text-slate-500">{new Date(p.created_at).toLocaleDateString('id-ID')}</p></td>
                                <td className="px-5 py-3.5"><p className="text-sm font-medium text-slate-700 dark:text-slate-300">{p.supplier?.name || 'Walk-in'}</p></td>
                                <td className="px-5 py-3.5 text-right"><p className="text-sm font-bold text-slate-800 dark:text-slate-200">Rp {Number(p.total).toLocaleString('id-ID')}</p></td>
                                <td className="px-5 py-3.5"><span className={`inline-flex px-2.5 py-0.5 text-xs font-medium rounded-full border ${statusStyles[p.status] || 'bg-slate-50 dark:bg-slate-700/50 text-slate-400 dark:text-slate-500'}`}>{p.status}</span></td>
                                <td className="px-5 py-3.5 text-right">
                                    <div className="flex items-center justify-end gap-1">
                                        <button onClick={() => setShowDetail(p)} className="p-1.5 rounded-lg text-slate-400 dark:text-slate-500 hover:text-primary-600 dark:hover:text-primary-400 hover:bg-primary-50 dark:hover:bg-primary-900/20 transition"><Eye size={16} /></button>
                                        {p.status === 'pending' && (
                                            <button onClick={() => handleReceive(p.id)} className="p-1.5 rounded-lg text-slate-400 dark:text-slate-500 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition" title="Receive Stock"><PackageCheck size={16} /></button>
                                        )}
                                    </div>
                                </td>
                            </tr>
            ...
            {showDetail && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-lg p-6 mx-4">
                        <div className="flex justify-between items-center mb-5">
                            <div><h2 className="text-lg font-bold text-slate-800 dark:text-white">Purchase Detail</h2><p className="text-xs text-slate-400 dark:text-slate-500">{showDetail.invoice_number}</p></div>
                            <button onClick={() => setShowDetail(null)} className="p-1.5 rounded-lg text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition"><X size={18} /></button>
                        </div>
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div><p className="text-xs font-medium text-slate-400 dark:text-slate-500">Status</p><span className={`inline-flex px-2.5 py-0.5 text-xs font-medium rounded-full border mt-1 ${statusStyles[showDetail.status] || ''}`}>{showDetail.status}</span></div>
                            </div>
                            {showDetail.items?.map((item, idx) => (
                                <div key={idx} className="flex justify-between items-center py-2 border-b border-slate-50 dark:border-slate-700/30 last:border-0">
                            ...
                            <div className="pt-3 border-t border-slate-100 dark:border-slate-700 space-y-1">
                                ...
                                <div className="flex justify-between text-sm pt-2 border-t border-slate-100 dark:border-slate-700"><span className="font-bold text-slate-800 dark:text-white">Total</span><span className="font-bold text-primary-600 dark:text-primary-400">Rp {Number(showDetail.total).toLocaleString('id-ID')}</span></div>
                            </div>
                            <button onClick={() => setShowDetail(null)} className="w-full py-3 text-sm font-medium text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-700 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-600 transition">Close</button>
                        </div>
                    </div>
                </div>
            )}
